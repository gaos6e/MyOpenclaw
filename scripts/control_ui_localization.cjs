...
const EXACT_TEXT_TRANSLATIONS = Object.freeze({
  ...
  "unauthorized: gateway token missing (open the dashboard URL and paste the token in Control UI settings)": "未授权：缺少网关令牌（请打开仪表盘地址，并将令牌粘贴到 Control UI 设置中）",
  Connected: "已连接",
  "Last connect": "上次连接",
  Yes: "是",
  No: "否",
  ...
  "How often beta-channel checks run in hours (default: 1).": "Beta 通道检查的运行频率（单位：小时，默认：1）。",
  performance: "性能",
  "Auto Update Enabled": "自动更新已启用",
  "Enable background auto-update for package installs (default: false).": "为软件包安装启用后台自动更新（默认：false）。",
  "Auto Update Stable Delay (hours)": "自动更新稳定通道延迟（小时）",
  "Minimum delay before stable-channel auto-apply starts (default: 6).": "稳定通道自动应用开始前的最小延迟（默认：6）。",
  "Auto Update Stable Jitter (hours)": "自动更新稳定通道抖动（小时）",
  "Extra stable-channel rollout spread window in hours (default: 12).": "稳定通道发布扩散窗口增加时长（单位：小时，默认：12）。",
  "Update Channel": "更新通道",
  ...
  "CLI Banner": "CLI 横幅",
  "CLI startup banner controls for title/version line and tagline style behavior. Keep banner enabled for fast version/context checks, then tune tagline mode to your preferred noise level.": "用于控制 CLI 启动横幅的标题/版本行与标语显示方式。建议保持横幅开启以便快速确认版本和上下文，再按你偏好的信息密度调整标语模式。",
  'Controls tagline style in the CLI startup banner: "random" (default) picks from the rotating tagline pool, "default" always shows the neutral default tagline, and "off" hides tagline text while keeping the banner version line.': "控制 CLI 启动横幅中的标语显示方式：`random`（默认）从轮换标语池中随机选择，`default` 始终显示中性的默认标语，`off` 则隐藏标语文本但保留版本行。",
  "Instrumentation, OpenTelemetry, and cache-trace settings": "监控埋点、OpenTelemetry 和缓存跟踪设置",
  "Cache Trace": "缓存跟踪",
  "Cache-trace logging settings for observing cache decisions and payload context in embedded runs. Enable this temporarily for debugging and disable afterward to reduce sensitive log footprint.": "用于观察嵌入式运行中缓存决策和载荷上下文的缓存跟踪日志设置。建议仅在调试时临时启用，完成后关闭以减少敏感日志暴露。",
  ...
  'Enable targeted diagnostics logs by flag (e.g. ["telegram.http"]). Supports wildcards like "telegram.*" or "*".': "按标记启用定向诊断日志（例如 `[\"telegram.http\"]`）。支持如 `telegram.*` 或 `*` 这样的通配符。",
  ...
  "OpenTelemetry Traces Enabled": "OpenTelemetry 追踪已启用",
  "Master toggle for diagnostics instrumentation output in logs and telemetry wiring paths. Keep enabled for normal observability, and disable only in tightly constrained environments.": "用于控制日志与遥测链路中诊断埋点输出的总开关。正常可观测性场景建议保持启用，仅在资源或合规要求非常严格的环境下再关闭。",
  "Enables OpenTelemetry export pipeline for traces, metrics, and logs based on configured endpoint/protocol settings. Keep disabled unless your collector endpoint and auth are fully configured.": "根据已配置的端点和协议设置启用 OpenTelemetry 的追踪、指标和日志导出链路。只有在采集器端点和鉴权都配置完成后再启用。",
  "Enable log signal export through OpenTelemetry in addition to local logging sinks. Use this when centralized log correlation is required across services and agents.": "除了本地日志输出外，同时通过 OpenTelemetry 导出日志信号。在需要跨服务和代理进行集中式日志关联时启用。",
  "OpenTelemetry export settings for traces, metrics, and logs emitted by gateway components. Use this when integrating with centralized observability backends and distributed tracing pipelines.": "用于配置网关组件导出的追踪、指标和日志的 OpenTelemetry 设置。在接入集中式可观测性后端和分布式追踪链路时使用。",
  "Collector endpoint URL used for OpenTelemetry export transport, including scheme and port. Use a reachable, trusted collector endpoint and monitor ingestion errors after rollout.": "用于 OpenTelemetry 导出的采集器端点 URL，包含协议和端口。请使用可达且可信的采集器端点，并在启用后关注数据接收错误。",
  "Interval in milliseconds for periodic telemetry flush from buffers to the collector. Increase to reduce export chatter, or lower for faster visibility during active incident response.": "定期将遥测数据从缓冲区刷新到采集器的间隔时间（毫秒）。调大可减少导出频率，调小则能在故障处理中更快看到数据。",
  "Additional HTTP/gRPC metadata headers sent with OpenTelemetry export requests, often used for tenant auth or routing. Keep secrets in env-backed values and avoid unnecessary header sprawl.": "随 OpenTelemetry 导出请求一起发送的额外 HTTP/gRPC 元数据请求头，通常用于租户鉴权或路由。请把敏感值放在环境变量中，并避免无必要地扩张请求头。",
  'OTel transport protocol for telemetry export: "http/protobuf" or "grpc" depending on collector support. Use the protocol your observability backend expects to avoid dropped telemetry payloads.': "遥测导出的 OTel 传输协议，可根据采集器支持情况选择 `http/protobuf` 或 `grpc`。请使用你的可观测性后端所要求的协议，避免遥测数据被丢弃。",
  ...
  "0 items": "0 项",
  Add: "添加",
  'No items yet. Click "Add" to create one.': "暂无条目。点击“添加”来创建。",
  "Forward Exec Approvals": "转发执行审批",
  "Approval Forwarding Mode": "审批转发模式",
  "Groups exec-approval forwarding behavior including enablement, routing mode, filters, and explicit targets. Configure here when approval prompts must reach operational channels instead of only the origin thread.": "用于配置执行审批转发行为，包括启用状态、路由模式、筛选条件和显式目标。当审批提示需要发送到运维频道而不仅是原始线程时，请在这里设置。",
  'Optional allowlist of agent IDs eligible for forwarded approvals, for example `["primary", "ops-agent"]`. Use this to limit forwarding blast radius and avoid notifying channels for unrelated agents.': "可选的代理 ID 白名单，只有这些代理的审批可被转发，例如 `[\"primary\", \"ops-agent\"]`。用它来限制转发范围，避免为无关代理通知频道。",
  "Enables forwarding of exec approval requests to configured delivery destinations (default: false). Keep disabled in low-risk setups and enable only when human approval responders need channel-visible prompts.": "启用后会把执行审批请求转发到已配置的投递目标（默认：false）。在低风险场景下建议保持关闭，仅在需要人工审批响应者通过频道可见提示处理时启用。",
  'Controls where approval prompts are sent: "session" uses origin chat, "targets" uses configured targets, and "both" sends to both paths. Use "session" as baseline and expand only when operational workflow requires redundancy.': "控制审批提示发送到哪里：`session` 使用原始聊天，`targets` 使用已配置目标，`both` 同时发送到两条路径。建议默认使用 `session`，仅在运维流程确实需要冗余时再扩展。",
  "Key bindings and shortcuts": "按键绑定与快捷方式",
  "Custom slash commands": "自定义斜杠命令",
  'Optional session-key filters matched as substring or regex-style patterns, for example `["discord:", "^agent:ops:"]`. Use narrow patterns so only intended approval contexts are forwarded to shared destinations.': "可选的会话键筛选规则，可按子串或类正则模式匹配，例如 `[\"discord:\", \"^agent:ops:\"]`。请尽量使用更窄的模式，确保只把预期的审批上下文转发到共享目标。",
  "Explicit delivery targets used when forwarding mode includes targets, each with channel and destination details. Keep target lists least-privilege and validate each destination before enabling broad forwarding.": "当转发模式包含 targets 时，这里定义显式投递目标，每项都包含频道和目标明细。请保持目标列表最小权限，并在启用广泛转发前逐一验证目标是否正确。",
  "Top-level binding rules for routing and persistent ACP conversation ownership. Use type=route for normal routing and type=acp for persistent ACP harness bindings.": "用于路由和持久化 ACP 会话归属的顶层绑定规则。普通路由使用 `type=route`，持久化 ACP 控制链路使用 `type=acp`。",
  "Defines elevated command allow rules by channel and sender for owner-level command surfaces. Use narrow provider-specific identities so privileged commands are not exposed to broad chat audiences.": "按频道和发送者定义提权命令的允许规则，用于 owner 级命令入口。请使用更窄的 provider 身份范围，避免高权限命令暴露给过宽的聊天对象。",
  session: "会话",
  targets: "目标",
  both: "两者",
  advanced: "高级",
  ...
  "Cron：Retry Backoff (ms)": "Cron：重试退避（毫秒）",
...
const EXACT_ATTRIBUTE_TRANSLATIONS = Object.freeze({
  ...
  Authentication: "身份验证",
  Cli: "CLI",
  "Form view can't safely edit some fields": "表单视图无法安全编辑某些字段",
  "Remove entry": "删除条目",
