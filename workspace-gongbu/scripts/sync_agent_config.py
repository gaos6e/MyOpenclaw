...
def main():
    cfg = {}
    try:
        cfg = json.loads(OPENCLAW_CFG.read_text(encoding='utf-8-sig'))
    ...
    has_taizi_runtime = any((ag.get('id') == 'taizi') for ag in agents_list)

    result = []
    seen_ids = set()
    for ag in agents_list:
        ag_id = ag.get('id', '')
        if ag_id == 'main' and has_taizi_runtime:
            continue
        ...
        if not allow_agents:
            allow_agents = ALLOW_AGENTS_FALLBACK.get(ag_id, [])
        result.append({
            'id': ag_id,
...
def sync_scripts_to_workspaces():
    ...
    for proj_name, runtime_id in _SOUL_DEPLOY_MAP.items():
        ...
        try:
            if ws_scripts.resolve() == scripts_src.resolve():
                continue
        except Exception:
            pass
    ...
    try:
        if ws_main_scripts.resolve() == scripts_src.resolve():
            return
    except Exception:
        pass
    for src_file in scripts_src.iterdir():
