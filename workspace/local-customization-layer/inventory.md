...
| Hindsight Guide | `workspace/hindsight_guide.md` | governance doc | 记录当前 Hindsight durable memory 的部署形态、模型拆分和运维入口 | 文档自身 | `hindsight-openclaw`、`workspace/scripts/ensure_hindsight_local.ps1`、`memory/hindsight-pg/` | 人工审阅 + retain/recall 实测 | 否 |
...
| Hindsight Local Runtime | `workspace/scripts/ensure_hindsight_local.ps1`、`workspace/scripts/stop_hindsight_local.ps1` | entrypoint | 启动和停止本机 `PostgreSQL + Hindsight API`，供 `hindsight-openclaw` 使用 | 脚本自身 + `.env` + `openclaw.json` | `memory/hindsight-pg/`、`memory/hindsight-api-venv/`、Qwen API | `workspace/scripts/hindsight_local_launcher.test.cjs` + retain/recall 实测 | 是 |
...
| Gateway Launchers | `gateway.cmd`、`gateway-hidden.ps1`、`gateway-hidden.vbs` | entrypoint | Windows 本机 gateway 启动入口，并在 gateway 前置拉起本地 Hindsight runtime | `openclaw.json` + 本机环境 | Node、OpenClaw 安装、`workspace/scripts/ensure_hindsight_local.ps1` | 人工启动验证 + `workspace/scripts/hindsight_local_launcher.test.cjs` | 是 |
