...
def test_sync_agent_config_falls_back_to_builtin_matrix(tmp_path, monkeypatch):
    sync_agent_config = _load_sync_agent_config()
    ...
    cfg = {
        "agents": {
            ...
            "list": [
                {
                    "id": "shangshu",
                    "workspace": str(tmp_path / "ws-shangshu")
                }
            ]
        }
    }
    ...
    out = json.loads((tmp_path / "data" / "agent_config.json").read_text())
    shangshu = next(agent for agent in out["agents"] if agent["id"] == "shangshu")
    assert shangshu["allowAgents"] == [
        "zhongshu",
        "menxia",
        "hubu",
        "libu",
...
def test_sync_agent_config_skips_main_when_taizi_exists(tmp_path, monkeypatch):
    sync_agent_config = _load_sync_agent_config()
    ...
    cfg = {
        "agents": {
            "defaults": {"model": "openai/gpt-4o"},
            "list": [
                {"id": "main", "workspace": str(tmp_path / "ws-main")},
