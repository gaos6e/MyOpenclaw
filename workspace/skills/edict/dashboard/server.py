...
def _resolve_openclaw_cmd():
    """Resolve a runnable OpenClaw CLI path for the current process.

    Windows scheduled/background Python processes often don't inherit the same
    PATH as an interactive shell, so relying on plain ``openclaw`` is fragile.
    Prefer the npm-installed ``openclaw.cmd`` shim when available.
    """
    global _OPENCLAW_CMD
    if _OPENCLAW_CMD:
        return _OPENCLAW_CMD

    candidates = []
    env_override = os.getenv('OPENCLAW_CMD', '').strip()
    if env_override:
    ...
    if os.name == 'nt':
        ...
        if appdata:
            candidates.extend([
                str(pathlib.Path(appdata) / 'npm' / 'openclaw.cmd'),
                str(pathlib.Path(appdata) / 'npm' / 'openclaw'),
            ])
        which_cmd = shutil.which('openclaw.cmd')
        if which_cmd:
            candidates.append(which_cmd)

    which_plain = shutil.which('openclaw')
    if which_plain:
        candidates.append(which_plain)

    for candidate in candidates:
        if candidate and pathlib.Path(candidate).exists():
            _OPENCLAW_CMD = candidate
            return _OPENCLAW_CMD

    _OPENCLAW_CMD = 'openclaw.cmd' if os.name == 'nt' else 'openclaw'
    return _OPENCLAW_CMD
...
def wake_agent(agent_id, message=''):
    ...
    def do_wake():
        try:
            cmd = [_resolve_openclaw_cmd(), 'agent', '--agent', runtime_id, '-m', msg, '--timeout', '120']
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    def _do_dispatch():
        try:
            ...
            cmd = [_resolve_openclaw_cmd(), 'agent', '--agent', agent_id, '-m', msg, '--timeout', '300']
            if _channel:
