...
test("patchHtmlCacheBust appends a stable cache-bust token to asset references", () => {
  const sample = `
  ...
  assert.equal(patched.changed, true);
  assert.match(patched.html, /\.\/assets\/index-ABC\.js\?v=token123/);
  assert.match(patched.html, /\.\/assets\/index-XYZ\.css\?v=token123/);
});
