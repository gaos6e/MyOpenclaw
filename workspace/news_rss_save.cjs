...
const feeds = [
  ...
  ['NHK','https://www3.nhk.or.jp/nhkworld/en/news/rss/'],
...
async function fetchWithTimeout(url, ms=18000){
  const ac=new AbortController(); const id=setTimeout(()=>ac.abort(new Error('timeout')), ms);
  try{ const res=await fetch(url,{headers:{'user-agent':'Mozilla/5.0'}, signal:ac.signal}); return await res.text(); }
...
(async()=>{
  ...
  for(const [name,url] of feeds){
    try{
      const xml=await fetchWithTimeout(url,18000);
      const items=extract(xml).filter(x=>x.title);
      results.push({name,url,items});
    }catch(e){
      results.push({name,url,error:String(e.message||e)});
    }
  }
  fs.writeFileSync('C:/Users/20961/.openclaw/workspace/news_results.json', JSON.stringify(results,null,2), 'utf8');
