...
function getGroupReplyState(groupId: number) {
    ...
    if (!state) {
        state = { lastBotReplyAt: 0, proactiveReplyTimestamps: [] };
        groupReplyStates.set(groupId, state);
    }
    return state;
}

function pruneProactiveReplyTimestamps(groupId: number, now: number) {
    const state = getGroupReplyState(groupId);
    state.proactiveReplyTimestamps = state.proactiveReplyTimestamps.filter((ts) => now - ts < PROACTIVE_HISTORY_WINDOW_MS);
    return state;
}
...
function hasAliasMatch(text: string, aliases: string[]): boolean {
    const normalizedText = normalizeTextForMatch(text);
    if (!normalizedText) return false;
    return aliases.some((alias) => normalizedText.includes(normalizeTextForMatch(alias)));
}
...
function recordGroupReply(groupId: number, now: number, proactive: boolean) {
    const state = pruneProactiveReplyTimestamps(groupId, now);
    state.lastBotReplyAt = now;
    if (proactive) {
        state.proactiveReplyTimestamps.push(now);
    }
}
...
export async function processInboundMessage(api: any, msg: OneBotMessage): Promise<void> {
    ...
    if (isGroup) {
        ...
        const aliasMatched = hasAliasMatch(textFromMsg, aliases);
        const repliedToBot = quotedSenderId != null && Number(quotedSenderId) === Number(selfId);
        const now = Date.now();
        const state = pruneProactiveReplyTimestamps(Number(msg.group_id), now);
        const recentBotParticipation = state.lastBotReplyAt > 0 && now - state.lastBotReplyAt <= CONTEXTUAL_REPLY_WINDOW_MS;
        const contextualMatched = (repliedToBot || recentBotParticipation) && looksLikeContextualFollowUp(textFromMsg);
        const triggerKeywords = getTriggerKeywords(cfg, config.accountId);
        const triggerMode = getTriggerMode(cfg);
        const keywordMatched = triggerKeywords.length > 0 && checkTriggerKeyword(textFromMsg, triggerKeywords, triggerMode);
        ...
        if (!decision.shouldReply) {
            api.logger?.info?.("[onebot] ignoring group message, no reply trigger matched");
            return;
        }
    ...
    let shouldRecordGroupReply = false;
    ...
    try {
        ...
        shouldRecordGroupReply = isGroup;
    } catch (err: any) {
        await clearEmojiReaction();
        ...
        if (shouldRecordGroupReply && groupId) {
            recordGroupReply(Number(groupId), Date.now(), triggerReason === "proactive");
        }
    }
}
