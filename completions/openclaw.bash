...
_openclaw_completion() {
    ...
    case "${prev}" in
      ...
      backup)
        ...
        ;;
      migrate)
        opts="list plan apply --from --include-secrets --overwrite --dry-run --yes --backup-output --no-backup --force --json"
        COMPREPLY=( $(compgen -W "${opts}" -- ${cur}) )
        return 0
        ;;
      doctor)
      ...
      exec-policy)
        ...
        ;;
      nodes)
        opts="status describe list pending approve reject remove rename invoke notify push canvas camera screen location "
        COMPREPLY=( $(compgen -W "${opts}" -- ${cur}) )
        return 0
        ;;
      devices)
      ...
      pairing)
        ...
        ;;
      plugins)
        opts="list inspect enable disable uninstall install update registry doctor marketplace "
        COMPREPLY=( $(compgen -W "${opts}" -- ${cur}) )
        return 0
        ;;
      channels)
